	...
	exports.showearningsPerPdt = function(req, res, next) {
		req.getConnection(function(err, connection) {
			if (err)
			   return next (err);
		       var query = "SELECT products.name, sum(sales_price * sales.no_sold) as earningsPerProduct FROM sales INNER JOIN products ON sales.prod_id = products.id GROUP BY products.name ORDER BY sum(sales_price) DESC;";
	...
	exports.showmostProfPdt = function(req, res, next) {
		req.getConnection(function(err, connection) {
			if (err)
			return next (err);
			var query = "SELECT products.name, sum(sales_price * sales.no_sold) as earningsPerProduct FROM sales INNER JOIN products ON sales.prod_id = products.id GROUP BY products.name ORDER BY earningsPerProduct DESC LIMIT 0,1;";
		    connection.query(query,[],function (err, results)	{
			if (err) {
				return (err);
			}
